...
This document guides students or instructors who are beginners with R and who want to learn calculus. It describes the use of the "R package" called `{mosaicCalc}`. The package provides software with a unified and consistent user interface for calculus operations. `{mosaicCalc}` is the engine behind the [*MOSAIC Calculus*](https://dtkaplan.github.io/MC2/) textbook, available free online. However, instructors can and do use `{mosaicCalc}` independently of [*MOSAIC Calculus*](https://dtkaplan.github.io/MC2/).

R, as you may know, is an environment and language for technical computing. Think of technical computing as serving the specialized needs of scientists and engineers. 
...
The pattern becomes clearer if we divide it into parts, one for each piece of information.
...
Almost always, the function description is the second element of a command, following the opening parenthesis. The description is always written in a particular format called a "tilde expression" which is described in the next section of this document.

Graphics tasks always have a third element specifying the bounds to be used in the graphic. Note that a comma separates the second and third elements of the command.
...
As you progress through a calculus course, you will encounter new calculus tasks. Each such task has a distinctive `{mosaicCalc}` task name. But the commands you construct to perform these tasks will always have a format similar to the example above. For instance, there is a calculus task called "anti-differentiation" which can be carried out with a command starting with `antiD()`, for instance

```{r}
antiD(x*sin(x) ~ x)
```

Even if you don't know what "anti-differentiation" is, you should be able to make sense of the structure of the command: a task name followed by a function description, with the appropriate punctuation. The computer produces a result in response to the command. In the `slice_plot()` example the response is a graph. In the `antiD()` example, the response is a new mathematical function.
...
"Functions" are a fundamental structure both in mathematics and in computer programming. Indeed, calculus is about the study and use of "continuous functions." ([*MOSAIC Calculus*](https://dtkaplan.github.io/MC2/) and every other calculus text explain what "continuous" means mathematically.) 
...
[*MOSAIC Calculus*](https://dtkaplan.github.io/MC2/) and `{mosaicCalc}` aim to avoid the ambiguity introduced in high school. A first step in doing this is to point out key distinctions that high-school notation confuses: a "formula" versus an "equation" versus a "function."
...
[*MOSAIC Calculus*](https://dtkaplan.github.io/MC2/) is careful to describe functions by specifying three things:
