...
If you are reading this document from within RStudio on the computer you will be using, chances are that `{mosaicCalc}` has already been installed. You can check by giving the R command `library(mosaicCalc)`. If that command gives an error message (check your spelling carefully!) you need to install `{mosaicCalc}` and some related packages. To do so, first check with your instructor if there is a specific process for your educational institution. (An example would be "Use rstudio.cloud.")  Otherwise, you can carry out the installation using the setup instructions in the *MOSAIC Calculus* textbook, via [this link](dtkaplan.github.io/MC2/R-mosaic-setup.html). 
...
**Reminder**: Whenever you open anew the R or RStudio apps, give the command needed to direct your R session to the `{mosaicCalc}` software:
