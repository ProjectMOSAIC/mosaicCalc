test_that("can pipe into contour plots", {
  P1 <- contour_plot(2*x - y ~ x & y, domain(x=-1:1, y=-1:1)) %>%
    contour_plot(x +2 * y ~ x & y, contour_color="red")
  P2 <- contour_plot(sin(x)*exp(-y) ~ x & y, domain(x=0:10,y=-1:1) ) %>%
    slice_plot(sin(x) ~ x)
...
test_that("order of layers doesn't matter", {
  ...
  P1 <- contour_plot(dt_rabbit(r,f) ~ r & f, domain(r=0.2:2, f=0.05:1), 
  ...
  expect_true(inherits(P1, "gg"))
  P2 <- contour_plot(dt_rabbit(r,f) ~ r & f, domain(r=0.2:2, f=0.05:1), 
                     contours_at = 0, contour_color = "red",
                     labels=FALSE) %>%
    vectorfield_plot(r ~ dt_rabbit(r,f),
                     f ~ dt_fox(r,f),
    ...
    contour_plot(dt_fox(r,f) ~ r & f,
                 ...
                 labels=FALSE) 
  expect_true(inherits(P2, "gg"))
  P3 <- vectorfield_plot(r ~ dt_rabbit(r,f), domain(r=0.2:2, f=0.05:1),
                     f ~ dt_fox(r,f),
                     transform=function(x) x^0.3, env=environment(dt_rabbit)) %>%
